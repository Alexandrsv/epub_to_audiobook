...
- For using *Silero TTS*, run the separate local Silero API described below.
...
- **Silero TTS**: Local Russian synthesis through a separately running service
...
Silero runs as a separate service; the existing Gradio UI in the `silero-tts`
project remains available independently. Start its FastAPI server:

```bash
python -m silero_tts.api
```

It listens on `http://127.0.0.1:8000` by default. Then select Silero in the WebUI or
run:

```bash
python3 main.py book.epub audiobook_output \
  --tts silero \
  --silero_base_url http://127.0.0.1:8000/v1 \
  --model_name v5_5_ru \
  --voice_name eugene
```

The Silero service performs mandatory Russian text normalization before splitting
text into model-sized fragments. `--output_text` still saves the prepared source
EPUB text, not the normalized service-side representation. Use `POST /normalize`
on the Silero service to inspect normalization.

This integration supports Russian-language text only.

**License notice:** Russian Silero v5/v5.5 models use
[CC BY-NC](https://github.com/snakers4/silero-models/blob/master/LICENSE) and are
intended for non-commercial use. Commercial use requires an appropriate Silero
license or another model/engine. See the
[Silero Models repository](https://github.com/snakers4/silero-models).
The MIT-licensed `v5_cis_base` model may be an alternative for commercial
scenarios, but it does not provide automatic stress placement or homograph
handling.
